    ...
    .then(function(result) {
      ...
      if (result.error) {
        ...
        form.addEventListener("submit", function(event) {
          ...
          cardSetup(stripe, card, result.intent.client_secret)
        })
      }
    })
}

const cardSetup = async function(stripe, card, clientSecret) {
  ...
  if (!paymentMethodId) {
    showError(`Invalid payment method id: ${paymentMethodId}`)
    return
  }
      ...
      .then(function(result) {
        if (result.error) {
          ...
          updateCard(stripe, {
            ...
            new_payment_method_id: result.setupIntent.payment_method
          })
        }
      })
  } catch (e) {
...
const updateCard = async function(stripe, payload)
{
  console.log(payload)
  
  await fetch("/card_change/update-payment-method", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({
      ...
      new_payment_method_id: payload.new_payment_method_id
    })
  })
    .then(function(result) {
      return result.json()
